...
def _write_material_carpaint(name, material):
    ...
    <shader name="{n}">
        ...
        <principled_bsdf name="{n}_main"
                         base_color = "{c.r} {c.g} {c.b}"
                         specular = "0.1"
                         roughness = "0.5"
                         clearcoat = "1.0"
                         clearcoat_roughness = "0.05" />
        <connect from="{n}_main bsdf" to="output surface"/>

        <!-- ColorRamp for noise -->
        <rgb_ramp name="{n}_noiseramp"
                  ramp="1.0 1.0 1.0 0.0 0.0 0.0"
                  ramp_alpha="0.0 0.4" />
        <connect from="{n}_noiseramp color" to="{n}_main metallic"/>

        <!-- Noise -->
        <noise_texture name="{n}_noise"
                       dimensions="3D"
                       scale="1000000"
                       detail="5"/>
        <connect from="{n}_noise fac" to="{n}_noiseramp fac"/>
    </shader>"""

    return snippet.format(
        n=name,
        c=material.carpaint.basecolor,
    )
